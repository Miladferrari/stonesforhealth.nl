 ...
 */
export async function generateInvoicePDF(order: any): Promise<Buffer | null> {
  try {
...
/**
 * Turn a succeeded PaymentIntent into a paid order: verify the amount, set the order to
 * processing and send the emails. Safe to call more than once (webhook retries, and the
 * thank-you page as a fallback when the webhook does not arrive).
 */
export async function fulfilPaidOrder(paymentIntent: Stripe.PaymentIntent, orderId: string): Promise<FulfilResult> {
  ...
  if (!check.ok) {
    console.error(`[Fulfil] Payment rejected for order ${orderId}: ${check.reason}`);
    if (check.order && ['pending', 'failed'].includes(check.order.status)) {
  ...
  // Don't mail the customer twice
  if (!['pending', 'failed'].includes(check.order.status)) {
    console.log(`[Fulfil] Order ${orderId} is already ${check.order.status}, skipping`);
